---
title: "hw4"
author: "Jordan Faires"
date: "5/21/2020"
output: html_document
---

sup!

...........sup? TAKE 2 

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r}
library(tidyverse)
```

```{r}

# Setting up variables and functions

# Temperature is increasing over time - temp at time t equals min of (tT / 100, T). It rises until T = 100 and flattens out after that. If we know t and T, you can plug in to the equation. t = years from now, T = BAU temperature increase at year 100.

# Save variables to make manipulation easier

# Year
t <- 50

# BAU temp increase at year 100
T <- 4.4

# Other paramaters
B <- 0.05
g <- 0.01
o <- 0.005
n <- 0.5

# Functions 

# Temperature in year T relative to temp at time 0

temp = function(t,T)
{
  out = min(t*T/100,T)
  return(out)
}

temp(t, T)

# Economic Activity - Activity reduces with heat over time. Fraction of economic activity retained in a year if tempt is r(t):

activity = function(B, temp)
{
  out = exp(-B*(temp^2))
  return(out)
}

activity(B, temp = temp(t, T))

# Consumption - economic activity growing over time at rate g but reduced by K. total consumption at time t is: 

consumption = function(activity, g, t)
{
  out = activity*(exp(g*t))
  return(out)
}

consumption(g, t, activity = activity(B, temp = temp(t, T))) 

# Utility - society's utility from consumption 

utility = function(consumption, n)
{
  out = ((consumption^(1-n))/(1-n))
  return(out)
}

utility(n, consumption = (consumption(g, t, activity = activity(B, temp = temp(t, T)))))

# Discount rate given by Ramsey Rule 

discount = function(o, n, g)
{
  out = o + (n*g)
  return(out)
}

discount(o, n, g)


```

```{r}

# Create Vector

years <- c(0:100)

temps <- seq(0, 4.4, .044)

#Create df for years

years.df <- data.frame(years, temps)

plot(years, temps)

#Different variations of applying temp fuction to vector / year df

years2.f <- years.df%>% 
  mutate(temp = temp(years.df$years))

years.df <- mapply(temp, years.df$years)

df[] <- lapply(years.df, temp(T = 8))

# Seems to be a problem with how the arguments are called - it wants arguments for t and T, but we want the function to use years in the vector or df for t instead of it being given. 

# I think we need to make a dummy dataset for temperatures over the 100 years and then just plot that, see years.df above? Or maybe I'm not understanding the formula for temperature correctly. I have no idea what you're trying to do here.
```

```{r}
# Other idea for plotting temp, not working

x <- seq(0, 200, 1)
T <- 200
plot(x, temp(x, T))
```

